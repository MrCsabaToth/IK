...
#def test_log_search_():
#    arr = [-1, -3, 5]
#    pos = log_search(arr, -1, True)
#    assert pos == 0
...
def test_log_search_small_missing2(i):
    rng = random.SystemRandom()
    length = rng.randint(5, 20)
    arr = [rng.randint(0, 1000) for j in range(length)]
    arr.sort()
    pos = log_search(arr, arr[0] - i - 1, False)
    assert pos == -1


@pytest.mark.parametrize("i", range(3))
def test_log_search_big_missing2(i):
    rng = random.SystemRandom()
    length = rng.randint(5, 20)
    arr = [rng.randint(0, 1000) for j in range(length)]
    arr.sort()
    pos = log_search(arr, arr[-1] + i + 1, False)
    assert pos == length
